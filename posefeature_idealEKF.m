function X_estimation=posefeature_idealEKF(X0,P0,z0,U_noise,z_noise,Index,ODOM_noise,OBSV_noise,Xstate_gt)
...
for i =1:T_steps
    ...
    F=eye(6*N+6);
    if i==1
        F(3*N+4:3*N+6,1:3)=-skew(Xstate_gt{i}(1:3,3*N+4));
    else
        F(3*N+4:3*N+6,1:3)=-skew(Xstate_gt{i}(1:3,3*N+4)-Xstate_gt{i-1}(1:3,3*N+4));
    end
...
%     P_adX_W=Gn*bigODOM_noise*Gn';
    ...
    %The form of error xi is a 6(N+1)*1 column (xi_R,xi_Rp,xi_x,xi_p)
    for j=1:N_ob
        H(3*j-2:3*j,1:3)=-Xstate_gt{i}(1:3,1:3)';
        H(3*j-2:3*j,3*Index{i}.RemainIndex(j)+1:3*Index{i}.RemainIndex(j)+3)=Xstate_gt{i}(1:3,1:3)';
        

        H(3*N_ob+3*j-2:3*N_ob+3*j,1:3)=Xstate_gt{i}(1:3,1:3)'*skew(Xstate_gt{i}(1:3,3*N+4+Index{i}.RemainIndex(j))-X_Prediction(1:3,3*N+4));
        H(3*N_ob+3*j-2:3*N_ob+3*j,3*N+4:3*N+6)=-Xstate_gt{i}(1:3,1:3)';
        H(3*N_ob+3*j-2:3*N_ob+3*j,3*N+4+3*Index{i}.RemainIndex(j):3*N+6+3*Index{i}.RemainIndex(j))=Xstate_gt{i}(1:3,1:3)';
