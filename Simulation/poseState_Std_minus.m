function xi=poseState_Std_minus(X1,X2)
...
%xi is in the lie algebra for SO(3)^{N+1}*(R^3)^{N+1}. Its form is (xi_R;xi_Rp1;...;xi_RpN;xi_x;xi_p1;...;xi_pN)
%coded by Yang SONG
...
%         xi(3*N+3*i-2:3*N+3*i,1)=jacor_inverse(-xi(1:3,1))*X(:,3*N+i);
        xi(3*N+3*i-2:3*N+3*i,1)=X(:,3*N+i);
    end
end
