function X_estimation=posefeature_RIEKF(X0,P0,z0,U_noise,z_noise,Index,ODOM_noise,OBSV_noise)
% coded by Yang SONG
...
%     A(4:6,1:3)=skew( Xn(1:3,3*N+4) )*Xn(1:3,1:3);%this is for Lie group noise
    A(4:6,1:3)=skew( Xn(1:3,3*N+4)+Xn(1:3,1:3)*U_noise{i}.position )*Xn(1:3,1:3);%this is for the first-order integration noise
    A(4:6,4:6)=Xn(1:3,1:3);
    C=zeros(6*N,6);
    for j=1:N
        C(3*N+3*j-2:3*N+3*j,1:3)=skew( Xn(1:3,3*N+4+j) )*Xn(1:3,1:3);        
    end
...
%         Y(6*j-2:6*j,1)=z_noise{i}.position(1:3,j)-X_Prediction(1:3,1:3)'*(X_Prediction(1:3,3*N+4+j)-X_Prediction(1:3,3*N+4));
    ...
    Xn=poseState_plus(posestate_exp(K*Y),X_Prediction);
...
%         z_new=[z_noise{i}.rotation(1:3,3*N+1:end) z_noise{i}.position(1:3,N+1:end)];
        [Xn,Pn]=Augmentation(Xn,Pn,z_new,OBSV_noise);
        Pn=(Pn+Pn')/2;
    end
